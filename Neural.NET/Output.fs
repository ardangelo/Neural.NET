...
module private Output =
    ...
    let rec FeedForward (activation : Activations.ActivationRecord) (z : Vector<double> list) 
        (w : Matrix<double> list) (b : Vector<double> list) : Vector<double> list =

        if w.IsEmpty then z else
        let a = z.Head.Map(activation.act, Zeros.Include)
